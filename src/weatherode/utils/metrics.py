...
# def lat_weighted_mse_velocity_guess(mean, std, y, guess_delta_x, delta_x, vars, lat, mask=None):
    # normal_lkl = torch.distributions.normal.Normal(mean, 1e-3 + std)

    # lkl = -normal_lkl.log_prob(y)

    # loss_val = lkl.mean() + 0.0001 * (std ** 2).sum() + 0.0001 * ((guess_delta_x - delta_x) ** 2).sum()

    # return {"loss": loss_val}


def lat_weighted_mse_velocity_guess(pred, ode_pred, noise_pred, y, vars, lat, mask=None, gradient_loss=False, epoch=0):
    """Latitude weighted mean squared error
    ...
    if gradient_loss:
        if epoch < 2:
            ...
            error = (noise_pred - (y - ode_pred.detach())) ** 2
    else:
        error = (pred - y) ** 2

    # ============================= time dimension weights =============================
    # t = error.size(0)
    # indices = torch.arange(t, dtype=torch.float32)
    # weights = torch.sigmoid((indices - (t-1) / 2) / ((t-1) / 8)).to(error.device).view(t, 1, 1, 1, 1)
    # error *= weights
    # ===================================================================================

    error = error.reshape(-1, error.shape[2], error.shape[3], error.shape[4])
    ...
    # loss_dict["velocity_guess"] = ((guess_delta_x - delta_x) ** 2).mean()

    # loss_dict["variable_loss"] = (error * w_lat.unsqueeze(1)).mean(dim=1).mean()

    if mask is not None:
        loss_dict["loss"] = ((error * w_lat.unsqueeze(1)).mean(dim=1) * mask).sum() / mask.sum() # + ((guess_delta_x - delta_x) ** 2).mean()
    else:
        loss_dict["loss"] = (error * w_lat.unsqueeze(1)).mean(dim=1).mean() # + ((guess_delta_x - delta_x) ** 2).mean()
