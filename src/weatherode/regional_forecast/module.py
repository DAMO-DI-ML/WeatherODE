...
from weatherode.regional_forecast.ode import RegionalWeatherODE
...
class RegionalForecastModule(LightningModule):
    """Lightning module for regional forecasting with the WeatherODE model.
    ...
    def training_step(self, batch: Any, batch_idx: int):
        x, y, predict_ranges, variables, out_variables, region_info = batch
        ...
        lat = self.lat[min_h:max_h+1]
        lon = self.lon[min_w:max_w+1]

        loss_dict, _ = self.net.forward(
            x, y, predict_ranges, variables, out_variables, [lat_weighted_mse_velocity_guess], lat=lat, lon=lon, epoch=self.current_epoch
        )
